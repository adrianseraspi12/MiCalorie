...
class MainDataSource implements DataSource {
  ...
  Future<Result<List<Food>>> getAllFood(int mealId) async {
    var listOfFood = await _foodRepo.findAllDataWith(mealId);
  ...
  Future<Result<List<MealNutrients>>> getAllMealNutrients(
    ...
    var listOfMealNutrients = await _mealNutrientsRepo.findAllDataWith(totalNutrientsPerDayId);
    ...
    return Success<List<MealNutrients>>(listOfMealNutrients);
  }
  ...
  Future<Result> removeFood(int mealId, Food food) async {
    ...
    var totalNutrientsPerDayId = currentMeal.totalNutrientsPerDayId;
  ...
  Future<Result<MealNutrients>> updateOrInsertFood(
    ...
    var updatedMealNutrients =
        await _updateMeal(totalNutrientsPerDay.id, food, mealNutrients, newCount);
    await _insertFood(updatedMealNutrients.id, food, newCount);
  ...
  Future _removeFoodFromTotalNutrients(
    ...
    final totalNutrientsPerDay = await _totalNutrientsPerDayRepo.getDataById(totalNutrientsId);
  ...
  Future _insertFood(int mealId, Food food, int newCount) async {
    final currentFood = await _foodRepo.getDataById(food.id);
    Food updateFood;
